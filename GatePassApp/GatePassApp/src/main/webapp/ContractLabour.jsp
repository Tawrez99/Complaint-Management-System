...
function capturePhoto() {
  ...
  const canvas = document.getElementById("canvas");
  const preview = document.getElementById("photoPreview");
  const hiddenField = document.getElementById("photoData");

  const context = canvas.getContext("2d");
  context.drawImage(video, 0, 0, canvas.width, canvas.height);
  const dataUrl = canvas.toDataURL("image/png");

  hiddenField.value = dataUrl; // Store base64 image in hidden input
...
function retrieveSerNoDetails() {
  ...
  var urlRefNoDetails = "/visitor/LabourGatePassSerNoDetail?refNo=" + refNo;
      ...
      <td width="60%">
        <form action="/saveContractLabourDetails" method="post" 
          ...
          <table border="0" cellpadding="6">
            ...
            <tr><td>Name</td>
                <td><input type="text" id="name" name="name" onkeyup="capLtr(this.value,'name');"/></td></tr>

            <tr><td>Father Name</td>
                <td><input type="text" id="fatherName" name="fatherName" onkeyup="capLtr(this.value,'fatherName');"/></td></tr>

            <tr><td>Designation</td>
                <td><input type="text" id="desig" name="desig" onkeyup="capLtr(this.value,'desig');"/></td></tr>

            <tr><td>Age</td>
                <td><input type="text" id="age" name="age"/></td></tr>

            <tr><td>Local Address</td>
                <td><input type="text" id="localAddress" name="localAddress" onkeyup="capLtr(this.value,'localAddress');"/></td></tr>

            <tr><td>Permanent Address</td>
                <td><input type="text" id="permanentAddress" name="permanentAddress" onkeyup="capLtr(this.value,'permanentAddress');"/></td></tr>

            <tr><td>Contractor’s Name / Address</td>
                <td><input type="text" id="contrctrNameAddress" name="contrctrNameAddress" onkeyup="capLtr(this.value,'contrctrNameAddress');"/></td></tr>

            <tr><td>Vehicle No.</td>
                <td><input type="text" id="vehicleNumber" name="vehicleNumber" onkeyup="capLtr(this.value,'vehicleNumber');"/></td></tr>

            <tr><td>Identification Mark</td>
                <td><input type="text" id="identification" name="identification" onkeyup="capLtr(this.value,'identification');"/></td></tr>

            <tr><td>Validity Period</td>
                <td>
                  From: <input type="text" name="valdity_fromDate"/>  
                  <a onclick="javascript:NewCal('valdity_fromDate','ddmmmyyyy');">
                    <img src="/visitor/image/cal.gif"/></a>
                  To: <input type="text" name="valdity_toDate"/>  
                  <a onclick="javascript:NewCal('valdity_toDate','ddmmmyyyy');">
                    <img src="/visitor/image/cal.gif"/></a>
                </td>
            </tr>
